...
class PathFinder(object):
    ...
    def initialize_single_source(self, nodes, source):
        
        ancestor_dict = dict()
        node_dist_pairs_dict= dict()
        
        for node in nodes:
            new_pair = NodeDistancePair(node, float('inf'))
            node_dist_pairs_dict[node] = new_pair
            ancestor_dict[new_pair.node] = None
        
        node_dist_pairs_dict[source].distance = 0
        return node_dist_pairs_dict, ancestor_dict
    
    def relax(self, u, v, w):
        u_d, v_d = u.distance, v.distance
        
        if v_d > (u_d + w(u.node, v.node)):
            v.distance = u.distance + w(u.node, v.node)
            v_pi = u.node
            return v_pi
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        """
        node_dist_pairs_dict, ancestor_dict = self.initialize_single_source(nodes, source)
        visited = set()
        priority_queue = PriorityQueue()
        is_done = False
        
        for _, node_pair in node_dist_pairs_dict.items():
            priority_queue.insert(node_pair)
            
        while len(priority_queue) > 0 and (not is_done):
            ...
            for v in u.node.adj:
                ...
                v_pair = node_dist_pairs_dict[v]
                new_v_pi = self.relax(u, v_pair, weight)
                
                if new_v_pi != None:
                    priority_queue.decrease_key(v_pair)
                    ancestor_dict[v] = new_v_pi
