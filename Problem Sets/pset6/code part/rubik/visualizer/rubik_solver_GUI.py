...
###############################################
    ...
    def fill_cell_properties(self, button, x, y):
        ...
        i = x % 2 + y % 2 * 2
        button["bg"]  = self.getColor(self.faces[int(face)][i])
        button["activebackground"] = self.colors_hover[self.getColor(self.faces[int(face)][i])]
        return 0
    ...
    def getFaceColor(self, face):
        return self.getColor(self.faces_color[int(face)])
...
############ GUI COMPONENTS ############
    ...
    def update_cell(self, btn):
        ...
        self.fill_cell_properties(btn, self.faces_pos[int(face)][0] + index % 2, self.faces_pos[int(face)][1] + index / 2)
    ...
    def change_cell_color(self, btn, new_color):
        if (len(self.steps) == 0):
            ...
            if (face == "D" and index == 3):
                ...
                self.lblInfo["text"] = ""
                self.faces[int(face)][index] = self.colors[new_color]
                self.update_cell(btn)
        else:
            self.lblInfo["text"] = "Please Reset before editing colors."
